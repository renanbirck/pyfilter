...
class TestAnalog(unittest.TestCase):
    ...
    def test_compute_butter_bp(self):
        ...
        self.assertAlmostEqual(butter.A[8], 4405040750.916997, places=2)
        self.assertAlmostEqual(butter.A[9], 28647635164.403412, places=2)
        self.assertAlmostEqual(butter.A[10],
                               174871956719.38678)
        self.assertAlmostEqual(butter.A[11],
                               760477697837.74438)
        self.assertAlmostEqual(butter.A[12],
                               3179819056953.7114, places=1)
        self.assertAlmostEqual(butter.A[13],
                               7534656938190.1543, places=1)
        self.assertAlmostEqual(butter.A[14],
                               19130579538158.508, places=1)
        ...
        self.assertAlmostEqual(butter.A[10],
                               208373474926.16934, places=1)
        self.assertAlmostEqual(butter.A[11],
                               905920861700.2373, places=1)
        self.assertAlmostEqual(butter.A[12],
                               3558965506031.5576, places=1)
        self.assertAlmostEqual(butter.A[13],
                               8442608469993.4551, places=1)
        self.assertAlmostEqual(butter.A[14],
                               19130579538158.492)
    ...
    def test_compute_freqs(self):
        """ Try computing the points for the plot. """
        butterworth = analog.ButterworthFilter()
        butterworth.filter_kind = 'lowpass'
        butterworth.N = 2
        butterworth.Wn = 100  # 100 rad/si
        butterworth.design()
        butterworth.compute_frequencies(N=50)
        target_freqs = [10, 10.98541142, 12.06792641, 13.25711366, 14.56348478,
                        15.9985872, 17.57510625, 19.30697729, 21.20950888,
                        23.29951811, 25.59547923, 28.11768698, 30.88843596,
                        33.93221772, 37.2759372, 40.94915062, 44.98432669,
                        49.41713361, 54.28675439, 59.63623317, 65.51285569,
                        71.9685673, 79.06043211, 86.85113738, 95.40954763,
                        104.81131342, 115.13953993, 126.48552169, 138.94954944,
                        152.64179672, 167.68329368, 184.20699693, 202.35896477,
                        222.29964825, 244.20530945, 268.26957953, 294.70517026,
                        323.74575428, 355.64803062, 390.69399371, 429.19342601,
                        471.48663635, 517.94746792, 568.9866029, 625.05519253,
                        686.64884, 754.31200634, 828.64277285, 910.29817799,
                        1000.]

        target_values = [0.98990101-0.14140722j, 0.98778822-0.15533456j,
                         0.98522755-0.17063006j, 0.98212153-0.18742601j,
                         0.97835039-0.20586617j, 0.97376658-0.22610606j,
                         0.96818782-0.24831262j, 0.96138822-0.27266303j,
                         0.95308702-0.29934201j, 0.94293437-0.32853673j,
                         0.93049353-0.36042781j, 0.91521894-0.39517409j,
                         0.89643026-0.43288788j, 0.87328324-0.47359552j,
                         0.84474105-0.51717625j, 0.80955392-0.56327058j,
                         0.76626321-0.61114844j, 0.71325863-0.65953181j,
                         0.64893416-0.70638068j, 0.57200200-0.74868579j,
                         0.48201588-0.78237309j, 0.38008680-0.80250229j,
                         0.26960986-0.80397511j, 0.15659024-0.78283687j,
                         0.04905377-0.73786678j, -0.04465356-0.67167879j,
                         -0.11811790-0.59050325j, -0.16852104-0.50252882j,
                         -0.19686500-0.41565403j, -0.20687805-0.33578939j,
                         -0.20343138-0.26626863j, -0.19124425-0.20817404j,
                         -0.17418155-0.16106117j, -0.15506023-0.12367137j,
                         -0.13574867-0.09445107j, -0.11737639-0.07186141j,
                         -0.10054943-0.05452957j, -0.08552801-0.04130164j,
                         -0.07235761-0.03124263j, -0.06095929-0.02361265j,
                         -0.05118885-0.01783519j, -0.04287398-0.0134657j ,
                         -0.03583665-0.01016377j, -0.02990581-0.00766999j,
                         -0.02492402-0.00578729j, -0.02075033-0.00436632j,
                         -0.01726089-0.00329403j, -0.01434835-0.00248497j,
                         -0.01192056-0.00187457j, -0.00989901-0.00141407j]

        for idx, coef in enumerate(target_freqs):
            self.assertAlmostEqual(butterworth.W[idx], coef, places=3)

        for idx, coef in enumerate(target_values):
            self.assertAlmostEqual(butterworth.H[idx], coef, places=3)
